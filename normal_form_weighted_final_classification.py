...
stop_words_list = ["перед", "килограмм" , "литр", "вес", "год", "паст"]
...
#функция определяющая веса для слов в названии позиции в чеке
#например, существительное 'NOUN' в именительном падеже 'nomn' имеет  
...
# преобразуем строку, повторяя каждое слово в соответствии с его весом
...
df_OKP['case'] = df_OKP['clean_form'].apply(case_fun)
...
#формируем список, где первый элемент - позция из чека, для которой рассчитывается косинусное расстояние с каждым элементом списка из ОКП
    
#
    the_corpus = [item] + list(df_OKP_main["normal_form_weighted"])


# функция для векторизации данных
    vec = TfidfVectorizer()

# матрица количества вхождений слова в конкретный документ (tfidf)
    X = vec.fit_transform(the_corpus)

# рассчитываем попарное косинусное расстояние между документами (X[0:1] - первый елемент списка, для которого считается кос. расстояние со всеми остальными элементами) 
    S = cosine_similarity(X[0:1], X)
    coef_list = S.ravel().tolist()

    max_value_index= coef_list.index(max(coef_list[1:len(coef_list)])) # выбрираем max, начиная с элемента 1, т.к. элемент с индексом 0 - это позиция из чека, для которой ищем группу с наибольшим опдобием

#TODO
#проверить корректность опеределения индекса в "the_corpus" и "df_OKP_main"            
...
df_item_result = df_item [['Item', 'POS', 'case', 'multiplicator', 'группа ОКП (по косин расстоянию','оригинальное название группы ОКП', 'номер гурппы ОКП']]


df_item_result.to_csv('C:\\Users\\msmirnov\\Documents\\Проект Spark\\df_item_result_weights_genitive.csv', sep=';', encoding='ansi')
...
# рассчитываем попарное косинусное расстояние между документами (формируется квадратная матрица для элементов "каждый-с -с каждым") 
    ...
    max_value_index= S.ravel().tolist().index(max(S.ravel().tolist()))
